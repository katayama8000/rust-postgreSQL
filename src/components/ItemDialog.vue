<template>
  ...
  >
    <v-card>
      <v-card-title @click="xxx">{{ titleText }}</v-card-title>
      <v-divider/>
      <v-card-text>
        <v-form ref="form" v-model="valid">
          ...
          <!-- 特定の条件時(クリック時・ホバー時のみなど）のみポップアップする場合のトリガー用のスロット -->
            ...
            >
              ...
              <v-btn text color="purple" @click="$refs.menu.save(date)">選択</v-btn>
            </v-date-picker>
             <!-- クリック時に出てくるカレンダー END ------------------------------------------------------------------------->
          </v-menu>
          <!-- タイトル -->
...
export default {
  ...
  methods: {
    ...
    open (actionType, item) {
      this.show = true
      // actionType:編集の時はeditが渡される
      //item:編集の時は、すでに入れられた値が入っている
